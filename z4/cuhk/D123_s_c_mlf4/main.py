...
class Main():
    ...
    def vis_stn(self, num):
        ...
        if num == 2:
            theta1, theta2 = extract_theta(model, torch.unsqueeze(data.stn_image, 0), num)
            print(theta1)
            print(theta2)
            x1 = w/2*(1+theta1[0,2]-theta1[0,0])
            y1 = h/2*(1+theta1[1,2]-theta1[1,1])
            cv2.rectangle(img, (x1, y1), (x1+w*theta1[0,0]-1, y1+h*theta1[1,1]-1), (0,0,255), 2)
            x2 = w/2*(1+theta2[0,2]-theta2[0,0])
            y2 = h/2*(1+theta2[1,2]-theta2[1,1])
            cv2.rectangle(img, (x2, y2), (x2+w*theta2[0,0]-1, y2+h*theta2[1,1]-1), (255,0,0), 2)
            
            cv2.imwrite("stn/stn_2_{}.png".format(opt.stn_image.split('/')[-1]), img)

        if num == 3:
            theta1, theta2, theta3 = extract_theta(model, torch.unsqueeze(data.stn_image, 0), num)
            print(theta1)
            print(theta2)
            print(theta3)
            x1 = w/2*(1+theta1[0,2]-theta1[0,0])
            ...
            # cv2.imwrite("stn/s1/stn_3_1_{}.png".format(opt.stn_image.split('/')[-1]), img[int(y1):int(y1+h*theta1[1,1]-1),:])
            # cv2.imwrite("stn/s1/stn_3_2_{}.png".format(opt.stn_image.split('/')[-1]), img[int(y2):int(y2+h*theta2[1,1]-1),:])
            # cv2.imwrite("stn/s1/stn_3_3_{}.png".format(opt.stn_image.split('/')[-1]), img[int(y3):int(y3+h*theta3[1,1]-1),:])
            
            # cv2.imwrite("stn/s2/stn_3_1_{}.png".format(opt.stn_image.split('/')[-1]), img[:int(h/3)+1,:])
            # cv2.imwrite("stn/s2/stn_3_2_{}.png".format(opt.stn_image.split('/')[-1]), img[int(h/3)+1:int(h*2/3)+1,:])
            # cv2.imwrite("stn/s2/stn_3_3_{}.png".format(opt.stn_image.split('/')[-1]), img[int(h*2/3)+1:,:])
            
            cv2.rectangle(img, (x1, y1), (x1+w*theta1[0,0]-1, y1+h*theta1[1,1]-1), (0,0,255), 2)
            cv2.rectangle(img, (x2, y2), (x2+w*theta2[0,0]-1, y2+h*theta2[1,1]-1), (0,255,255), 2)
            cv2.rectangle(img, (x3, y3), (x3+w*theta3[0,0]-1, y3+h*theta3[1,1]-1), (255,0,0), 2)
            
            cv2.imwrite("stn/stn_3_{}.png".format(opt.stn_image.split('/')[-1]), img)
