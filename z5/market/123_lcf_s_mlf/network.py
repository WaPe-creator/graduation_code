...
class M2N(nn.Module):
    def __init__(self):
        ...
        self.mlf1 = MultiLayerFusionBlock(256, 2048)
        self.mlf2 = MultiLayerFusionBlock(512, 2048)
        self.mlf3 = MultiLayerFusionBlock(1024, 2048)
        ...
        self.avgpool_m = nn.AvgPool2d(kernel_size=(12, 4))
        self.reduction_m  = nn.Sequential(nn.Conv2d(2048, feats, 1, bias=False), nn.BatchNorm2d(feats), nn.ReLU())
        self._init_reduction(self.reduction_m)
        self.fc_m = nn.Linear(feats, num_classes)
        self._init_fc(self.fc_m)
    ...
    def forward(self, x):
        ...
        xm = m1 + m2 + m3 + xg
        pm = self.avgpool_m(xm)
        pm = self.reduction_m(pm).squeeze(dim=3).squeeze(dim=2)
        fm = self.fc_m(pm)
        ...
        predict = torch.cat([pm, pg, pr2, pr3], dim=1)

        return predict, pm, pg, pr2, pr3, fm, fg, f2_1, f2_2, f3_1, f3_2, f3_3
